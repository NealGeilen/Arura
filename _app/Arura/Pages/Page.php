...
class Page{
    ...
    public function showPage(){
        ...
        if (empty($this->getMasterPath())){
            ...
            self::getSmarty()->assign('sContent', include ($this->getFileLocation() . '/' . basename($this->getFileLocation()). '.php'));
...
//          Loading page sections
            foreach (scandir($this->getMasterPath() . 'Sections/') as $item){
                if (pathinfo($this->getMasterPath() .'Sections/'. $item, PATHINFO_EXTENSION) === 'html'){
                    $sName = str_replace('.html', '', $item);
                    self::getSmarty()->assign($sName, self::getSmarty()->fetch($this->getMasterPath() .'Sections/'. $item));
                }
            }
